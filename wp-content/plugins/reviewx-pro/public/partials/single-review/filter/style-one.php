...
function reviewx_review_filter_query_html( $args ) {
	...
	if( \ReviewX_Helper::check_post_type_availability( $args['post_type'] ) == TRUE ) {
		...
		$allow_share    		= $settings->allow_share_review;
		$allow_like_dislike    	= $settings->allow_like_dislike;
		$shop_icon_url    		= $settings->icon_upload;
		
	} else if( $args['post_type'] == 'product' ) {

		$review_per_page        = isset($args['number']) ? $args['number'] : get_option( '_rx_option_review_per_page' );
    ...
    if( is_array( $comments ) && count( $comments ) ) {
        ...
        foreach ($comments as $comment) {
			...
			if( 0 == $comment->comment_parent ) {
				...
				if( $args['tag'] == 'text' &&  $comment_video_url != '' ) {
						...
						<div class="rx_author_info">
							<div class="rx_thumb"> 
								<?php 
									...
									\ReviewX_Helper::get_gravatar( $comment_object, $args['post_type'] );									
								?> 
							</div>
									...
									<div class="rx_body">										
										...
										<div class="comment-body">
											<div class="comment-content">
												<?php echo comment_text($child_comment); ?>
											</div>
										</div> 										
									</div>								
								</div>
		...
		if( isset($args['pagination']) && ($args['pagination'] == 'on' ||  $args['pagination'] != 'off' || $args['pagination'] == '') ) {
			...
			<script>
				jQuery(document).ready(function($) {
					...
					//video popup
					$('.rx-popup-video, .popup-vimeo').magnificPopup({
						disableOn: 700,
						type: 'iframe',
						mainClass: 'mfp-fade',
						removalDelay: 160,
        ...
        echo '<div class="rx_no_review">';
			...
			} else if( $args['tag'] == "video" ) {
				echo esc_html__( 'There is no video review yet.', 'reviewx-pro' );            
			} else {
				echo esc_html__( 'There is no review yet.', 'reviewx-pro' );
			}
