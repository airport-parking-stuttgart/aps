<?php
    ...
    if( get_post_type( get_the_ID() ) == 'product' ) {
        ...
        $form_position          = get_option( '_rx_option_review_form_position' );           
       ...
       $allow_video_filter      = \ReviewX_Helper::get_post_meta( $reviewx_id, 'allow_video', true ); 
       $allow_multiple_review   = \ReviewX_Helper::get_post_meta( $reviewx_id, 'allow_multiple_review', true );
       $review_style    		= \ReviewX_Helper::get_post_meta( $reviewx_id, 'review_style', true );  
       $allow_share             = \ReviewX_Helper::get_post_meta( $reviewx_id, 'allow_share_review', true ); 
       $allow_like_dislike      = \ReviewX_Helper::get_post_meta( $reviewx_id, 'allow_like_dislike', true );
       $shop_icon_url    		= \ReviewX_Helper::get_post_meta( $reviewx_id, 'icon_upload', true );  
       ...
       $filter_video  		    = \ReviewX_Helper::get_post_meta( $reviewx_id, 'filter_video', true );   
       ...
       $form_position           = \ReviewX_Helper::get_post_meta( $reviewx_id, 'review_form_position', true ); 
    }
?>

    <?php if( $form_position == 'top' ): ?>
        <?php include 'review-form-one.php'; ?>
    <?php endif; ?>   
    ...
    if ( have_comments() ) :
        $post_id                    = get_the_ID();
        $rx_total_reviewer          = rx_total_reviewer( $post_id, '' );    
    
    if( shortcode_divi_review_list(get_the_ID(), $reviewx_shortcode) && shortcode_divi_review_filter(get_the_ID(), $reviewx_shortcode) ) { ?> 
                ...
                <?php  if( get_post_type( get_the_ID() ) == 'product' ) { ?>
                    <?php
                        if( $rx_total_reviewer > 0) {
                            echo sprintf( __( 'Reviewed by %02d customer(s)', 'reviewx-pro' ), $rx_total_reviewer );
                        } else {
                            echo sprintf( __( 'Reviewed by %d customer(s)', 'reviewx-pro' ), $rx_total_reviewer );
                        }
                    ?>
                <?php } else if( \ReviewX_Helper::check_post_type_availability( get_post_type( get_the_ID() ) ) == TRUE ) { ?>
                    <?php 
                        if( $rx_total_reviewer > 0) {
                            echo sprintf( __( 'By %02d reviewer(s)', 'reviewx-pro' ), $rx_total_reviewer ); 
                        } else {
                            echo sprintf( __( 'By %d reviewer(s)', 'reviewx-pro' ), $rx_total_reviewer ); 
                        }                        
                    ?>
                <?php } ?>
            </h4>
        </div>
        <?php if( $filter_recent == 1 || $filter_photo == 1 || $filter_text == 1 || $filter_top_rated == 1 || $filter_low_rated == 1 || $filter_video == 1 ) { ?>
		<div class="rx-short-by">
			<h4><?php echo ! empty(get_theme_mod('reviewx_sort_by_label') ) ? get_theme_mod('reviewx_sort_by_label') : __( 'Sort by', 'reviewx-pro' ); ?></h4>
			<div class="rx_review_shorting_2">
				<div class="box">
					<select class="rx_shorting" name="rx_shorting">
                        <?php if( $filter_recent == 1 ) { ?>
                            <option value="recent"><?php echo __( 'Recent Review', 'reviewx-pro' ); ?></option>
                        <?php } ?>
                        <?php if( $allow_img_filter == 1 && $filter_photo == 1 ) { ?>
                            <option value="photo"><?php echo __( 'Photo Review', 'reviewx-pro' ); ?></option>
                        <?php } ?>
                        <?php if( $allow_video_filter == 1 && $filter_video == 1) { ?>
                            <option value="video"><?php echo __( 'Video Review', 'reviewx-pro' ); ?></option>
                        <?php } ?>
                        <?php if( $filter_text == 1 ) { ?>
                            <option value="text"><?php echo __( 'Text Review', 'reviewx-pro' ); ?></option>
                        <?php } ?>
                        <?php if( $filter_top_rated == 1 ) { ?>
                            <option value="rating"><?php echo __( 'Top Rated', 'reviewx-pro' ); ?></option>
                        <?php } ?>
                        <?php if( $filter_low_rated == 1 ) { ?>
                            <option value="low"><?php echo __( 'Low Rated', 'reviewx-pro' ); ?></option>
                        <?php } ?> 
					</select>
    ...
    <input type="hidden" class="rx_product_id" name="rx_product_id" value="<?php the_ID(); ?>">
    <input type="hidden" class="rx-product-rating" name="rx-product-rating" id="rx-product-rating">
    <input type="hidden" name="rx-sorting-post-type" class="rx-sorting-post-type" id="rx-sorting-post-type" value="<?php echo get_post_type( $post_id ); ?>">
    <?php if ( is_user_logged_in() ) { ?>
    <input type="hidden" name="rx-user-id" class="rx-user-id" id="rx-user-id" value="<?php echo get_current_user_id(); ?>">                    
    <?php } ?>  

    <?php  if( shortcode_divi_review_list(get_the_ID(), $reviewx_shortcode) ) { ?>    
            ...
            <div class="rx_listing_style_1">
                <ul class="rx_listing" id="rx-commentlist">
                    ...
                    if(isset($reviewx_shortcode) && !empty($reviewx_shortcode['rx_product_id'])){
                        $reviewx_shortcode['rx_post_type']  = get_post_type( $post_id );
                        $reviewx_shortcode['rx_product_id'] = $post_id;
                        ...
                        if( function_exists('reviewx_shortcode_query_args') ){
                    ...
                    if( ! empty( $comments2) ) {
                        ...
                        $comment_gallery_meta   = !empty( get_comment_meta( $comment->comment_ID, 'reviewx_attachments', true) ) ? get_comment_meta( $comment->comment_ID, 'reviewx_attachments', true) : [];
                        ...
                        if( 0 == $comment->comment_parent ) {
                            $pagination_item = 'rx-pagination-item';
                        ?>
                                ...
                                <div class="rx_flex rx_review_wrap">
                                    <div class="rx_author_info">
                                        <div class="rx_thumb"> 
                                            <?php 
                                                ...
                                                \ReviewX_Helper::get_gravatar( $comment_object, get_post_type( $post_id ) );									
                                            ?> 
                                        </div>
                                    ...
                                    <div class="rx_body">
                                        <div class="rx_flex rx_rating_section">
                                            ...
                                            <?php if( current_user_can('manage_options') && check_comment_parent_id( $comment->comment_ID ) == 0 && get_current_user_id() != get_comment_author_id( $comment->comment_ID ) ) : ?> 
                                                    ...
                                                    <?php 
                                                        if( ! empty( $rx_highlight ) ) {
                                                            ...
                                                            <span>   
                                                                ...
                                                                <?php echo __( 'Remove', 'reviewx-pro' ); ?>
                                                            </span>
                                                            <?php                                            
                                                        } else {
                                                            ?>
                                                            <span>
                                                                ...
                                                                <?php echo __( 'Highlight', 'reviewx-pro' ); ?>
                                                            </span>
                                                            <?php                                          
                                                        }
                                                    ?>                            
                                        ...
                                        <?php 
                                            ...
                                            ?>
                                                ...
                                                if( $allow_img_filter == 1 ) {				
                                                    foreach ( $comment_gallery_meta as $comment_gallery_id ) {
                            ...
                            <?php					
                                if( ! empty( reviewx_get_comment_by_id( $comment->comment_ID ) ) ) { 
                                    ...
                                    ?>
                                    <ul class="children">
                                        <li class="comment byuser comment-author-admin bypostauthor odd alt depth-2 rx_photo_review_item">
                                            <div class="rx_flex rx_review_wrap">
                                                <div class="rx_thumb">
                                                    <?php 
                ...
                <?php if( !isset($reviewx_shortcode) || (isset($reviewx_shortcode) && ( isset($reviewx_shortcode['rx_pagination']) ? $reviewx_shortcode['rx_pagination'] : '' == 'on') ) || (isset($reviewx_shortcode) && ( isset($reviewx_shortcode['rx_pagination']) ? $reviewx_shortcode['rx_pagination']:'' != 'off' ) ) ) { ?> 
                <?php if( get_post_type( $post_id ) == 'product' ) { ?>
                    <input type="hidden" id="rx-total-review" value="<?php echo \ReviewX_Helper::get_total_review( get_the_ID(), get_post_type( $post_id ), '', '', get_current_user_id() ); ?>">
    ...
    <input type="hidden" name="rx-highlight-nonce" class="rx-highlight-nonce" id="rx-highlight-nonce" value="<?php echo wp_create_nonce( "special-string" ); ?>">
    <?php } ?>
    <?php else : ?>
        <p class="woocommerce-noreviews"><?php esc_html_e( 'There are no reviews yet.', 'reviewx-pro' ); ?></p>
    <?php endif; ?>

    <?php if( $form_position == 'bottom' || $form_position == '' ): ?>
        <?php include 'review-form-one.php'; ?>
    <?php endif; ?>