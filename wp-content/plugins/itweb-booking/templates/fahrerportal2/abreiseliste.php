					...
					<tr style="background-color: <?php echo $order['Color'] ?> !important" export-color="<?php echo $order['Color'] ?>" class="row<?php echo $i % 2; ?>">
						...
						<td class="order-betrag" style="position:relative;">
							...
							<?php if (get_post_meta($order['Nr.'], '_transaction_id')[0] == "barzahlung") : ?>
								<input type="text" <?php echo $editOK; ?> style="width:100px; background: rgba(255, 255, 255, 0.5) !important" value="<?php echo $order['Betrag'];
																																																																			if ($order['Service'] != '0.00') echo " " . number_format($order['Service'], 2, '.', ''); ?>" class="form-control transparent-input"><span style="display: none;"><?php echo $order['Betrag'] ?></span>
							<?php else : ?>
