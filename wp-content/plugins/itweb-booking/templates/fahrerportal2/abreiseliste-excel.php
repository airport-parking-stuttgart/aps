...
if (isset($_SESSION['allorders'])) {
  ...
  foreach ($result as $r) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($j++, $rowCount, mb_strtoupper(date("d.m.Y",  strtotime($r->Abreisedatum)), 'UTF-8'));
    ...
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($j++, $rowCount, mb_strtoupper($r->FahrerAb, 'UTF-8'));
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($j++, $rowCount, mb_strtoupper($r->Sonstige_1, 'UTF-8'));
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($j++, $rowCount, mb_strtoupper($r->Sonstige_2, 'UTF-8'));
    $rowCount++;
  }
