<div class="page container-fluid <?php echo $_GET['page'] ?>">
	...
	<?php
		...
		$filter['datum_von2'] = isok($_GET, 'date_from') ? dateFormat($_GET['date_from']) : date('Y-m-d');
		$filter['datum_bis2'] = isok($_GET, 'date_to') ? dateFormat($_GET['date_to']) : date('Y-m-d');
...
// 		Database::getInstance()->mydd($filter['betreiber']);

		$allorders = Database::getInstance()->getBookings2($filter);

		// Database::getInstance()->mydd(count($allorders));
	}
				...
				<div class="col-sm-12 col-md-12 ui-lotdata">
					<div class="row">
						<div class="col-sm-12 col-md-3 col-lg-2">
							<input type="text" name="token" placeholder="Buchung" class="form-item form-control" value="<?php if ($token != "") echo $token;
																																																					else echo ''; ?>">
						</div>
						<div class="col-sm-12 col-md-1 col-lg-1 ui-lotdata-date">
							<input type="text" id="dateFrom" name="date_from" placeholder="Buchung von" class="form-item form-control single-datepicker" value="<?php if ($datefrom != "") echo date('d.m.Y', strtotime($datefrom));
																																																																									else echo ''; ?>">
						</div>
						<div class="col-sm-12 col-md-1 col-lg-1 ui-lotdata-date">
							<input type="text" id="dateTo" name="date_to" placeholder="Buchung bis" class="form-item form-control single-datepicker" value="<?php if ($dateto != "") echo date('d.m.Y', strtotime($dateto));
																																																																							else echo ''; ?>">
						</div>
						<div class="col-sm-12 col-md-1 col-lg-1 ui-lotdata-date">
							<input type="text" id="arrivaldateFrom" name="anreiseVon" placeholder="Anreise von" class="form-item form-control single-datepicker" value="<?php if ($anreiseVon != "") echo date('d.m.Y', strtotime($anreiseVon));
																																																																													else echo ''; ?>">
						</div>
						<div class="col-sm-12 col-md-1 col-lg-1 ui-lotdata-date">
							<input type="text" id="arrivaldateTo" name="anreiseBis" placeholder="Anreise bis" class="form-item form-control single-datepicker" value="<?php if ($anreiseBis != "") echo date('d.m.Y', strtotime($anreiseBis));
																																																																												else echo ''; ?>">
						</div>
					</div>
		...
		<?php if ($datefrom != "" && $dateto != "") : ?>
			...
			<? else : ?>
				...
				<tbody>
					...
					<?php foreach ($allorders as $booking) : ?>
						<?php
						// var_dump($booking);
                    ...
                    <?php
							...
							<td>
								<?php if ($additionalPrice != '0.00') echo number_format($additionalPrice, 2, '.', '');
								else echo '-' ?>
							</td>
							<td>
