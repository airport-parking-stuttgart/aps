...
if(isok($_GET, 'filter_an')){
	$kfilter = $_GET['filter_an'];
	$filter['filter_an'] = $kfilter;
}
...
// Database::getInstance()->mydd($_SESSION['allorders']);
// 		die;



// Database::getInstance()->mydd(get_site_url());
				...
				<div class="col-sm-12 col-md-12 ui-lotdata">
					<div class="row">
						...
						<div class="col-sm-12 col-md-3 col-lg-2">
							<input type="text" name="filter_an" placeholder="Buchung/Name" class="form-item form-control" value="<?php if($kfilter != "") echo $kfilter; else echo ''; ?>">
						</div>
...
<!--
		<div class="btn">
			<form action="<?= get_site_url() . '/wp-content/plugins/itweb-booking/templates/fahrerportal/anreiseliste-tcpdf.php'; ?>" method="post">
				<button type="submit" id="btnExport" value="Export to Excel" class="btn btn-success">TCPDF</button>
			</form>
		</div>
-->
			...
			<tbody>
				...
				foreach ($data as $order) : ?>
					...
					<tr style="background-color: <?php echo $order['Color'] ?> !important" export-color="<?php echo $order['Color'] ?>" class="row<?php echo $i % 2; ?>">
						...
						<input type="hidden" name="bezahlmethode" value="<?php echo get_post_meta($order['Nr.'], '_transaction_id', true) ?>">
						<td class="order-betrag" style="position: relative;">
							...
							//$currency = $betrag[1];
							?>
							<?php if (get_post_meta($order['Nr.'], '_transaction_id', true) == "barzahlung") : ?>								
