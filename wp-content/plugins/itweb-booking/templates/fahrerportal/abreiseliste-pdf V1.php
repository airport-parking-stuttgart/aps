...
foreach ($result as $key => $r) {
	...
	if(date('Y-m-d', strtotime(get_post_meta($r->order_id, 'Anreisedatum', true))) <= "2023-09-18" && ($r->Produkt == 80566 || $r->Produkt == 80567)){
		$bg_hl = " background-color: ". $color;
	}
	else{
		$bg_hl = "";
	}
    ...
    $html .= "<td style=' font-size: 12px; border: 1px solid black; padding:3px; white-space: nowrap; " . $border_top . $border_bottom . $bg_hl . "'>" . get_post_meta($r->order_id, 'token', true) . "</td>";	
