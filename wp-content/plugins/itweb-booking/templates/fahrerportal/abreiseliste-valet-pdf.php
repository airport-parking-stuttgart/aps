...
foreach ($result as $key => $r) {
	...
	if($zeile - ($umbruch + 1) == 0){
		$html .= "<h4 style='text-align:center'>Abreiseliste ".$_SESSION['service_type']." | " . $date . " | Seite " . $seite . " von " . $seiten_bis . "</h4>";
	...
	$html .= "<td style=' font-size: 12px; border: 1px solid black; padding:3px; white-space: nowrap;'>" . $r->FahrerAb . "</td>";
...
$filename = "Abreiseliste Valet  " . $_SESSION['dateFrom'] . " - " . $_SESSION['dateTo'];
...
$filePath = ABSPATH . 'wp-content/uploads/abreiseliste_valet.pdf';
$pdf = fopen($filePath, 'w');
fwrite($pdf, $file);
fclose($pdf);

$pdf_url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]";
echo "<script>location.href = '".$pdf_url."/wp-content/uploads/abreiseliste_valet.pdf';</script>";
