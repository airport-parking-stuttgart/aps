...
if(empty($_GET['site']) || $_GET['site'] == 1){
	...
	foreach($allorders as $booking){
		$duration = get_post_meta($booking->order_id, 'Anreisedatum', true) != null && get_post_meta($booking->order_id, 'Abreisedatum', true) != null && $booking->is_for != 'hotel' ? getDaysBetween2Dates(new DateTime(get_post_meta($booking->order_id, 'Anreisedatum', true)), new DateTime(get_post_meta($booking->order_id, 'Abreisedatum', true))) : 0;
...
if($_GET['site'] == 3){
	...
	foreach($allorders as $booking){
		$zeit =  date('H', strtotime(get_post_meta($booking->order_id, 'Uhrzeit von', true)));
		if($zeit >= 3 && $zeit < 6)
			$anreisezeit['03-06'] += 1;
	...
	$filter['buchung_bis'] = date('Y-m-d', strtotime($dateto . '+1 day'));
	$filter['orderBy'] = "Buchungsdatum";
	$allBookings = Database::getInstance()->get_bookinglist($filter);
