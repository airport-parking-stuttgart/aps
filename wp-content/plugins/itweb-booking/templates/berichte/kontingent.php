...
$sum_client_used = $sum_client_free = $sum_broker_used = $sum_broker_free = [];

// Initialize sum arrays for client products
...
// Initialize sum arrays for broker products
...
$sum_all_used = array_merge($sum_client_used, $sum_broker_used);
$sum_all_free = array_merge($sum_client_free, $sum_broker_free);
...
foreach ($period as $key => $value) {
    ...
    foreach ($parklots as $parklot) {
        $sql .= ",
            SUM(CASE WHEN pl.parklot_short = '".$parklot->parklot_short."' THEN 1 ELSE 0 END) AS 'used ".$parklot->parklot_short."'
        ";
    }
    ...
    $sql .= "
        ...
        LEFT JOIN ".$wpdb->prefix."posts s ON s.ID = o.order_id
        LEFT JOIN ".$wpdb->prefix."itweb_parklots pl ON o.product_id = pl.product_id
        WHERE DATE('".$value->format('Y-m-d')."') BETWEEN DATE(o.date_from) AND DATE(o.date_to)
            AND o.product_id = pl.product_id AND o.deleted = 0
            AND s.post_status = 'wc-processing'
    ";
...
//echo "<pre>"; print_r($parklots); echo "</pre>";
?>
